...
const CollaborationPanel = React.memo(({
    ...
    const onAction = (fn: () => void) => {
        ...
        // Avoid setTimeout delay which can cause focus loss during unmount
        try {
            fn();
        } finally {
                ...
                <ScrollArea className="flex-1">
                    <div className="p-6 space-y-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <Card className="bg-white/5 border-gray-800 p-5 space-y-4 hover:bg-white/[0.07] transition-all">
                                ...
                                <div className="flex gap-2">
                                    <Input
                                        ...
                                        className="bg-black/40 border-gray-700 h-9 transition-all focus:ring-1 focus:ring-blue-500 font-arabic text-right"
                                        value={teamNameInput}
                                        onChange={(e) => setTeamNameInput(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && onAction(handleCreateTeam)}
                                    />
                                    <Button
                                        type="button"
                            ...
                            <Card className="bg-white/5 border-gray-800 p-5 space-y-4 hover:bg-white/[0.07] transition-all">
                                <div className="space-y-1 text-right">
                                    <h3 className="text-sm font-bold text-green-400">الانضمام لفريق</h3>
                                    <p className="text-[10px] text-gray-500">أدخل كود الدعوة للانضمام لمشروع قائم.</p>
                                </div>
                                <div className="flex gap-2">
                                    <Input
                                        placeholder="كود الدعوة..."
                                        className="bg-black/40 border-gray-700 h-9 transition-all focus:ring-1 focus:ring-green-500 font-arabic text-right font-mono"
                                        value={inviteCodeInput}
                                        onChange={(e) => setInviteCodeInput(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && onAction(handleJoinTeam)}
                                    />
                                    <Button
                                        type="button"
                                        disabled={isProcessing}
